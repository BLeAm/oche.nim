...
class Oche {
  late final nmonteCarloPiNimCall = dynlib.lookupFunction<NmonteCarloPiNimN, NmonteCarloPiNimD>('monteCarloPiNim');
  ...
  List<int> mandelbrotNim(final int v_width, final int v_height, final int v_maxIter) {
      ...
      if (p.address == 0) return []; try {
        final L = ffi.Pointer<ffi.Int64>.fromAddress(p.address).value; final d = ffi.Pointer<ffi.Int64>.fromAddress(p.address + 16);
