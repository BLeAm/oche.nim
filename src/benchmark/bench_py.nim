...
type Dummy {.porche.} = object
  x: int

# CASE A: Monte Carlo Pi (Pure Logic - 10 Million Iterations)
proc monteCarloPiNim(iterations: int): float {.porche.} =
  var count = 0
  var x, y: float
...
proc mandelbrotNim(width, height, maxIter: int): seq[int] {.porche.} =
