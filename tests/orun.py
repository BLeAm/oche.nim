...
eq("getColor() == Green", oche.getColor(), 1)  # Green = 1

u_for_status = oche.makeUser("Alice", 30)
eq("getStatus(active user) == Active", oche.getStatus(u_for_status), 1)  # Active = 1
...
p3 = oche.addPoints(p, p2)   # pass User plain object
approx("addPoints x", p3.x, 11.0)
approx("addPoints y", p3.y, 22.0)

part = oche.makeParticle(1.0, 2.0, 3.0, 2)  # Blue = 2
...
msg = oche.greetUser(u)
eq("greetUser", msg, "Hello, Charlie! Age: 99")

# Nested struct
t = oche.makeTagged("origin", 0.0, 0.0)
...
u_plain = oche.makeUser("Dave", 40)
msg2 = oche.greetUser(u_plain)
eq("greetUser(User plain)", msg2, "Hello, Dave! Age: 40")

# Pass via view (from share buffer)
buf_u = oche.makeUserBuffer(3)
uv = buf_u[0]  # UserView
msg3 = oche.greetUser(uv)
...
users_copy = oche.makeUserList(4)
...
# context manager
with oche.makeIntBuffer(3) as tmp:
...
pbuf = oche.makeParticleBuffer(4)
...
src = oche.makeIntBuffer(5)   # [0, 3, 6, 9, 12]
total = oche.sumIntBuffer(src)
eq("sumIntBuffer", total, 30)

doubled = oche.doubleIntBuffer(src)
eq("doubleIntBuffer[0]", doubled[0], 0)
eq("doubleIntBuffer[1]", doubled[1], 6)
eq("doubleIntBuffer[4]", doubled[4], 24)
doubled.free()

ubuf2 = oche.makeUserBuffer(4)
count = oche.countActiveUsers(ubuf2)
...
dot = oche.dotProduct(a, b)
approx("dotProduct numpy arrays", dot, 2+6+12+20)

factor_buf = oche.multiplyArray(a, 3.0)
...
dot2 = oche.dotProduct(arr_aa, b2)
...
s = oche.sumIntsPtr(narr)
eq("sumIntsPtr numpy", s, 15)

neg = oche.negateIntsPtr(narr)
